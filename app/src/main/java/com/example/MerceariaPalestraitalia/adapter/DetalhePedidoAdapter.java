...
public class DetalhePedidoAdapter extends RecyclerView.Adapter<DetalhePedidoAdapter.MyViewHolder> {

    private final List<ItemPedido> itemPedidoList;
    private final Context context;

    public DetalhePedidoAdapter(List<ItemPedido> itemPedidoList, Context context) {
        ...
        produtoRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()){
                    ...
                    Picasso.get().load(produto.getUrlImagens().get(0).getCaminhoImagem()).into(holder.imgProduto);
                }else {
